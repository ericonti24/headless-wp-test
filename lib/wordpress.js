...
export const getPagesBySlug = async (slug) => {
  try {
    const url = `${baseURL}/index.php/wp-json/wp/v2/pages?slug=${slug}`;
    const response = await fetch(url, {
      mode: "no-cors", // Added here
    });

    // Since "no-cors" doesn't allow access to the response body, you can't parse JSON.
    // You'll need proper CORS headers for full functionality.
    if (!response.ok) {
      throw new Error(`Error fetching page with slug "${slug}": ${response.statusText}`);
    }
